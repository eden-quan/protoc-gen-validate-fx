...
func (m *Module) Execute(targets map[string]pgs.File, pkgs map[string]pgs.Package) []pgs.Artifact {
	...
	} else if langParamValue != "" && langParamValue != "go" {
	...
	for _, f := range targets {
		...
		for _, msg := range f.AllMessages() {
			m.CheckRules(msg)
			// Process Flatten
			msg2 := ProcessFlatten(msg)
			f2.allMessages = append(f2.allMessages, msg2)
		}
		...
		for _, tpl := range tpls {
			...
			if out != nil {
				...
				if opts := f.Descriptor().GetOptions(); opts != nil && opts.GetJavaMultipleFiles() && lang == "java" {
					...
					//m.AddGeneratorTemplateFile(outPath, tpl, interface{}(f2).(pgs.File))
					m.AddGeneratorTemplateFile(outPath, tpl, f2)
				}
			}
		}
